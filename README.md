...
# Import the dataset
data_path = "Dataset/Bank_Marketing_Dataset.arff"
...
#### Customer marital status with respect to their personal loan default status

__Hypothesis__: We know from prior analysis that most customers do not default on personal loans either. Thus we would expect the marital status to have little to no effect on whether or not the customer defaulted on their personal loans.

# Deposit choice vs. loan defaults
plt.figure(figsize = (15, 5))
sns.countplot(data = data, x = "marital", hue = "loan", palette = "dark:silver")

plt.title("Comparison between marital status and personal loan default status", fontsize = 25)
...
It would seem from the chart above, that more people, irrespective of marital status, are likely to default on personal loans as opposed to credit. In keeping with the prior chart befopre this though, most people do not default on personal loans either.



#### Customer marital status with respect to their housing loan default status

__Hypothesis__:

Housing loans are generally much heavier than the others. As such, there might be some influence exerted by marital status over the housing loan defaults.

# Marital status vs. housing loans defaults
plt.figure(figsize = (15, 7))
sns.countplot(data = data, x = "marital", hue = "housing", palette = "dark:silver", stat = "percent")

plt.title("Comparison between marital status and housing loan default status", fontsize = 25)
...
From the chart above, it is very obvious that of all the loan types, the housing loans are more likely to see deafults. As discussed previously, this may likely be due to the hoising loans being more substantial and being paid over a longer timeframe in general.

+ Married people are most likely to default compared to people of other marital status, but it is usually relatively random, within a sample of married people. This might make some sense as married people will likely have other responsibilities on their finances.

+ Divorced people are the least likely to default, though out of a sample of divorcees, predicting their default probability is an even more random endeavor. More analysis needs to be done on this before any reasonable inferences can be reached.

<h2 align="center">Relation between debt types and deposit decision</h2>

#### Customer deposit decision with respect to their credit default status

__Hypothesis__: It would make sense to put forward the hypothesis that customers who have defaulted on credit previously extended to them may not be in the best financial state to make investment deposits.

We can attempt to observe the veracity of this hypothesis by visualizing the customers' deposit decision based on whether or not they have credit defaults. This is done via a __grouped bar chart__.
...
__Observation__: From the chart above, it is difficult to observe any overall trend. However, it would seem that people at the __secondary__ level of education dominate a good number of the occupations. This might make some sense as a lot of people are educated in the present day, and there seems to be a higher premium on education.
